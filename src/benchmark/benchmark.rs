...
impl ArwahNamedTimer {
    pub fn arwah_start(name: &'static str) -> Self {
        Self { name, start: Some(Instant::now()), end: None }
    }
    ...
    pub fn arwah_summary(&self) -> String {
        ...
        for timer in &self.named_timers {
            if timer.start.is_some() && timer.end.is_some() {
                let runtime_secs = timer.end.unwrap().saturating_duration_since(timer.start.unwrap()).as_secs_f32();
...
mod tests {
    ...
    fn test_benchmark() {
        ...
        assert!(benchmark.arwah_summary().contains("\nArwah Benchmark Summary\ntest       | 0."));
