...
unsafe impl Send for ArwahCap {}

pub fn arwah_open(dev: &str, config: &ArwahConfig) -> Result<ArwahCap> {
    ...
    if handle.is_null() {
        ...
        bail!("[ ETA ]: Failed to open interface: {}", err.to_str()?);
    }
    ...
    if ret != 0 {
        ...
        bail!("[ ETA ]: Failed to activate interface: {}", err.to_str()?);
    }
    Ok(ArwahCap { handle })
}

pub fn arwah_open_file(path: &str) -> Result<ArwahCap> {
    let mut errbuf = [0; pcap_sys::PCAP_ERRBUF_SIZE as usize];
    ...
    if handle.is_null() {
        let err = unsafe { CStr::from_ptr(errbuf.as_ptr()) };
        bail!("[ ETA ]: Failed to open file: {}", err.to_str()?);
    }
    Ok(ArwahCap { handle })
}

pub fn arwah_default_interface() -> Result<String> {
    let mut errbuf = [0; pcap_sys::PCAP_ERRBUF_SIZE as usize];
    let dev = unsafe { pcap_sys::pcap_lookupdev(errbuf.as_mut_ptr()) };

    if dev.is_null() {
        let err = unsafe { CStr::from_ptr(errbuf.as_ptr()) };
        bail!("[ ETA ]: Failed to find interface: {}", err.to_str()?);
    }
...
impl ArwahCap {
    pub fn arwah_datalink(&self) -> i32 {
        unsafe { pcap_sys::pcap_datalink(self.handle) }
    }

    pub fn arwah_next_pkt(&mut self) -> Result<Option<ArwahPacket>> {
        ...
        match retcode {
            i if i >= 1 => {
                ...
                Ok(Some(ArwahPacket { data: packet.to_vec() }))
            }
            0 => bail!("[ ETA ]: timeout expired"),
