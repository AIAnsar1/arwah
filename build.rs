...
pub fn main() {
    ...
    let pb_linenr = arwah_ports_v(&fp_map);
    let payb_linenr = arwah_payloads_v(&fp_map);
    let map = arwah_port_payload_map(pb_linenr, payb_linenr);
    arwah_generate_code(map);
}

fn arwah_generate_code(port_payload_map: BTreeMap<Vec<u16>, Vec<u8>>) {
...
fn arwah_payloads_v(fp_map: &BTreeMap<i32, String>) -> BTreeMap<i32, Vec<u8>> {
    ...
    for (&line_nr, data) in fp_map {
        if data.contains('\"') {
            ...
            payb_linenr.insert(line_nr, arwah_parser(payloads.trim()));
        }
    }
...
fn arwah_parser(payload: &str) -> Vec<u8> {
